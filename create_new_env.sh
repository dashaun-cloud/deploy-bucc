...
if [ "$1" = "vsphere" ]; then
  ...
  credhub set --type value --name /concourse/$2/buccbox_ssh_pub  --value "$(credhub get -n /concourse/pa/buccbox_ssh_pub -j | jq .value.public_key)"
  credhub set --type value --name /concourse/$2/vcenter_network --value "$(credhub get -n /concourse/pa/vcenter_network -j | jq .value)"
